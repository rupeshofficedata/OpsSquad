...
- **The remaining tool integrations** — `git.diff/log`, `secrets.scan`, `lint.run`, `test.select/run`, `registry.push/pull`, `iac.scan`, `http.smoke_test`, `prometheus.query`, `alertmanager.read` are still `SimulatedTool` stubs — following the same pattern as kubectl/terraform/trivy/docker/helm/argocd in `app/tools/real.py`.
