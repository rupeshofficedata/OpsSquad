...
async def execute(slug: str, req: FlightplanExecuteRequest, user: User = Depends(current_user)):
    ...
    status = await execute_flightplan(run_id, flightplan, req.inputs, user_role=user.role)
...
async def approve(run_id: str, user: User = Depends(require_role("admin"))):
    ...
    status = await resume_flightplan(run_id, flightplan, run["inputs"] or {}, user_role=triggering_role)
    return {"status": status, "run_id": run_id}
