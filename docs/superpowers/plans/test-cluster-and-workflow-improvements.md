...
**P1: tools and permissions.** **Status: items 7, 8 (partly), 10, 11 built; 9 and 12 not done.** Built: `kubectl.logs` returns a bounded tail plus error lines (and `previous`); new `kubectl.events`, `kubectl.describe`, `prometheus.targets`, `registry.list`, `pagerduty.list`; `alertmanager.read` lists up to 10 alerts; NotFound errors on get/logs/describe/restart/scale list the real names ("Did you mean: deployment/postgres?"); `trivy.scan` refuses paths outside `/app`; every tool result passes a redaction wrapper (`tools/redact.py`) and final answers are redacted too; `k8s/05-rbac.yaml` narrowed (read-only view plus changes only to `redis` and `canary`, so the runtime can no longer scale or restart itself, bff or postgres). Verified live in the test namespace by swapping in the narrow Role: reads, redis scale/restart and helm upgrade/rollback work, bff scale and postgres restart are Forbidden. **Not yet applied to the dev cluster** (needs `kubectl apply -f k8s/05-rbac.yaml`, a runtime image rebuild, and `UPDATE agents SET tools = ...` for the assistant row: the seed only inserts, so existing databases must be updated by hand until versioned migrations exist). Found by the P1 eval and fixed: (a) a local-model failure after tools had already run replaced the whole answer with a simulated one, discarding real results; now the real results are returned with the failure reason (a failure before any tool still falls back to simulated, badge shown). (b) The 8192-token context filled up (34 tool schemas + pod list + events + two `describe` outputs) and the reply was silently cut mid-sentence; history budget lowered to 10000 chars, `describe` capped at 2000 chars, and `finish_reason: length` now appends a visible "answer cut off" warning. Test-env `LOCAL_LLM_READ_TIMEOUT=300`. Eval (Bonsai PTQ1_0, test namespace): Q01, Q06, Q16, Q17, Q26-Q29 pass. Bonsai is slow on multi-tool questions (Q01 300 s, Q26 350 s). Not done: 9 (single result envelope, prompt generated from the registry) and 12 (`env` semantics); `kubectl.contexts` skipped (in-cluster runtime has no context name).
